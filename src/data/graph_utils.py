...
def build_nx_graph_from_edge_index(
    edge_index, edge_attr, tickers=None, sectors=None, drop_self_loops=True
):
    G = nx.Graph()
...
def graph_stats_over_time_from_returns(
    returns_tensor, topk, corr_w, step=10, tickers=None, sectors=None, use_abs=True
):
    import pandas as pd
    ...
    for t in range(0, T, step):
        ...
        edge_index, edge_attr = corr_graph_topk(
            Rw, topk=topk, use_abs=use_abs, add_self_loops=False
        )
        G = build_nx_graph_from_edge_index(
            edge_index, edge_attr, tickers=tickers, sectors=sectors, drop_self_loops=True
        )
