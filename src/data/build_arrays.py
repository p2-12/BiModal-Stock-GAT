...
from .price_features import engineer_features, FEATURE_COLS, future_log_return_curve, compute_regime_features
...
def download_prices(tickers, start, end, cache_path=None, auto_adjust=True, batch_size=20, max_retries=4, base_sleep=0.5, jitter=0.25, inter_batch_sleep=0.05):
    """
    ...
    for bi, batch in enumerate(batches):
        for t in batch:
            tcache=_cache_piece_path(cache_path, f"ticker.{t}") if cache_path is not None else None
            ...
            df=_defeatbeta_price_one(t, start, end, max_retries=max_retries, base_sleep=base_sleep, jitter=jitter)
            per[t]=df
...
def _market_proxy_from_universe(per_ticker_frames):
    ...
    if len(closes)==0:
        raise RuntimeError("Cannot build market proxy: no usable Close series in universe download.")
...
def build_graph_arrays(
    tickers, start, end, lookback, horizon, stride, use_text,
    news_lookback_days=3, finbert_model="ProsusAI/finbert",
    cache_prices=None, max_text_length=256,
):
    """
    ...
    if len(idxs)==0:
        raise RuntimeError("No snapshots available: increase date range or reduce lookback/horizon/stride.")
